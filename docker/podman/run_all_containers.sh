...
echo -e "${BLUE}==== Running All Agent Containers with Host Networking ====${NC}"

# Directory paths
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

# Define agent groups - using the ones that were built
GROUPS=("core-services" "memory-system" "utility-services" "ai-models-gpu-services" "language-processing")
...
# 2. Launch containers with host networking
...
# 3. Add GPU support for specific containers (rebuild them with devices)
if [ -e /dev/nvidia0 ]; then
    ...
    echo -e "${YELLOW}Starting ai-models-gpu-services with GPU support...${NC}"
    ...
    echo -e "${YELLOW}Starting language-processing with GPU support...${NC}"
...
echo -e "${YELLOW}Showing logs from core-services container:${NC}"
sudo podman logs -f --tail=10 core-services &
LOGS_PID=$!
sleep 5
