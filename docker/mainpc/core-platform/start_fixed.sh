#!/bin/bash
# =============================================================================
# MAINPC CORE PLATFORM FIXED STARTUP SCRIPT
# =============================================================================

set -e

echo "🚀 Starting MainPC Core Platform Container (FIXED)..."
...
start_agent() {
    ...
    cd /app
    python3 "${agent_script}" --port "${port}" \
    ...
    if kill -0 $(cat "/app/logs/core-platform/${agent_name}.pid") 2>/dev/null; then
        ...
        if [ -n "${health_port}" ]; then
            ...
            if curl -f "http://localhost:${health_port}/health" >/dev/null 2>&1; then
                ...
                echo "⚠️ ${agent_name} health check not responding yet"
            fi
        fi
    else
...
# 2. SystemDigitalTwin (system monitoring) - with different health port
start_agent "SystemDigitalTwin" "main_pc_code/agents/system_digital_twin.py" "7220" "7221"

# 3. RequestCoordinator (request routing) - with different health port
            ...
            if ! kill -0 "$pid" 2>/dev/null; then
                ...
                case "$agent_name" in
                    "ServiceRegistry")
                        ...
                        ;;
                    "SystemDigitalTwin")
                        start_agent "SystemDigitalTwin" "main_pc_code/agents/system_digital_twin.py" "7220" "7221" || all_running=false
                        ;;
                    "RequestCoordinator")
