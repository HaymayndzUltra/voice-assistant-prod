...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(log_dir, 'proactive_context_monitor.log')),
...
# ZMQ ports
PROACTIVE_MONITOR_PORT = int(os.getenv('PROACTIVE_MONITOR_PORT', '8573'))  # Changed to avoid conflicts
...
class ProactiveContextMonitor:
    ...
    def __init__(self):
        ...
        logger.info("Proactive Context Monitor initialized successfully")
    ...
    def _init_zmq(self):
        ...
        try:
            ...
            logger.info(f"Health check server started on port {PROACTIVE_MONITOR_HEALTH_PORT}")
    ...
    def _setup_health_check_server(self):
        ...
        server = HTTPServer(('localhost', PROACTIVE_MONITOR_HEALTH_PORT), HealthCheckHandler)
    ...
    def run(self):
        """Main run loop."""
        logger.info("Proactive Context Monitor starting...")
        try:
            while self.running:
                try:
