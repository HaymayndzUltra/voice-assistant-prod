...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(Path(__file__).parent.parent, 'logs', 'language_translation_coordinator.log'))
    ]
)
...
PC2_IP = _agent_args.host  # Temporarily changed for local testing
...
TAGABERT_SERVICE_ADDRESS = f"tcp://{_agent_args.host}:{TAGABERT_SERVICE_PORT}"
...
class LanguageAndTranslationCoordinator(BaseAgent):
    def __init__(self, port: int = None, **kwargs):
        ...
        # Input socket (from StreamingSpeechRecognition)
        self.input_socket = self.context.socket(zmq.SUB)
        self.input_socket.connect(f"tcp://{_agent_args.host}:{WHISPER_ASR_PORT}")
        self.input_socket.setsockopt(zmq.SUBSCRIBE, b"")
        
        # Output socket (to TTS)
        self.output_socket = self.context.socket(zmq.PUB)
        self.output_socket.bind(f"tcp://*:{TTS_CONNECTOR_PORT}")  # Bind to port 5564 as configured in startup_config.yaml
        ...
        try:
            ...
            # Quick ping to check TagaBERTa service
            self.tagabert_socket.send_json({'action': 'ping'})
            if self.tagabert_socket.poll(1000, zmq.POLLIN):
                self.tagabert_socket.recv_json() # consume ping response
                self.tagabert_available = True
                logger.info(f"Successfully connected to TagaBERTa service at {TAGABERT_SERVICE_ADDRESS}")
            else:
                logger.warning(f"TagaBERTa service at {TAGABERT_SERVICE_ADDRESS} did not respond to ping.")
                self.tagabert_socket.close()
                self.tagabert_socket = None # Ensure it's None if not available
        except zmq.error.ZMQError as e:
            logger.warning(f"Could not connect or initialize TagaBERTa service: {e}")
            if self.tagabert_socket:
                 self.tagabert_socket.close()
                 self.tagabert_socket = None
        except Exception as e:
            logger.warning(f"An unexpected error occurred during TagaBERTa service initialization: {e}")
            if self.tagabert_socket:
                 self.tagabert_socket.close()
                 self.tagabert_socket = None

        # Optional fastText model loading
        ...
        if FASTTEXT_AVAILABLE:
            try:
                ...
                if model_path.exists():
                    ...
                    logger.info(f"Loaded fastText LID model from {FASTTEXT_MODEL_PATH}")
                else:
                    logger.warning(f"fastText model file not found at {FASTTEXT_MODEL_PATH}")
            except Exception as e:
                logger.warning(f"Could not load fastText LID model: {e}")

        # Load custom Tagalog lexicon
        self.tagalog_lexicon = set()
        try:
            lexicon_p = Path(TAGALOG_LEXICON_PATH)
            if lexicon_p.exists():
                extra_words = {w.strip().lower() for w in lexicon_p.read_text(encoding="utf-8").splitlines() if w.strip()}
                self.tagalog_lexicon.update(extra_words)
                TAGALOG_WORDS_SET.update(extra_words) # Also update the global set
                logger.info(f"Loaded {len(extra_words)} words from Tagalog lexicon {TAGALOG_LEXICON_PATH}")
            else:
                logger.warning(f"Tagalog lexicon file not found at {TAGALOG_LEXICON_PATH}")
        except Exception as e:
            logger.warning(f"Could not load Tagalog lexicon {TAGALOG_LEXICON_PATH}: {e}")
    ...
    def start(self):
        ...
        self._running = True
        self._thread = threading.Thread(target=self.process_loop)
        self._thread.daemon = True
        self._thread.start()
    ...
    def cleanup(self):
        """Cleanup resources."""
        logger.info("Cleaning up Language and Translation Coordinator resources...")
        if self.performance_monitor:
            self.performance_monitor.stop()
        ...
        if self.tagabert_socket:
            self.tagabert_socket.close()
        # self.context.term() # Commented out as context termination might be handled globally
        logger.info("Language and Translation Coordinator resources cleaned up.")
...
if __name__ == "__main__":
    try:
        coordinator = LanguageAndTranslationCoordinator()
        logger.info("Language and Translation Coordinator service started")
        ...
        logger.info("Shutting down Language and Translation Coordinator service...")
