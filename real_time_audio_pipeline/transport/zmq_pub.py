...
from .schemas import (
    TranscriptEvent, EventNotification, serialize_for_zmq,
    EventTypes, create_event_notification
)
...
class ZmqPublisher:
    ...
    async def _publish_event_internal(self, event: EventNotification) -> None:
        ...
        try:
            ...
            topic = f"event.{event.event_type}".encode('utf-8')
    ...
    async def _publish_transcript_internal(self, transcript: TranscriptEvent) -> None:
        ...
        try:
            ...
            # Publish with topic for filtering
            topic = f"transcript.{transcript.language}".encode('utf-8')
