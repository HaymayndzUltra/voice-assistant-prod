...
# Script to launch a PC2 container with GPU support for the AI System
set -e

# Function to display usage information
function show_usage {
    echo "Usage: $0 <container-group> [--dev]"
    echo "  container-group: The group of agents to run (pc2-core-services, pc2-memory-services, etc.)"
    echo "  --dev: Mount the current directory for development (optional)"
    echo ""
    echo "Available container groups:"
    echo "  pc2-core-services: Essential PC2 system services"
    echo "  pc2-security-services: Authentication and security services"
    echo "  pc2-memory-services: Memory and context management services"
    echo "  pc2-task-services: Task processing and routing services"
    echo "  pc2-ai-services: Specialized AI services (requires GPU)"
    echo "  pc2-web-services: Web and filesystem services"
    exit 1
}
...
VALID_GROUPS=("pc2-core-services" "pc2-security-services" "pc2-memory-services" "pc2-task-services" "pc2-ai-services" "pc2-web-services")
VALID_GROUP=0
...
# First, make sure GPU support is working if needed
if [[ "$CONTAINER_GROUP" == "pc2-ai-services" ]]; then
    echo "Checking GPU device paths in WSL2 for GPU-enabled container..."
...
ENV_VARS="-e CONTAINER_GROUP=$CONTAINER_GROUP -e PYTHONPATH=/app -e PYTHONUNBUFFERED=1 -e PC2_MODE=true"

# Add GPU support flags if needed
GPU_FLAGS=""
if [[ "$CONTAINER_GROUP" == "pc2-ai-services" ]]; then
    GPU_FLAGS="--device nvidia.com/gpu=all"
fi

echo "Launching PC2 container $CONTAINER_NAME from image $IMAGE_NAME..."
