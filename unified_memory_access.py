...
class UnifiedMemoryManager:
    ...
    def get_session_context(self) -> Dict[str, Any]:
        ...
        context = {'timestamp': datetime.now().isoformat(), 'provider': self.provider_type, 'session_data': self._session_context}
        try:
...
if __name__ == '__main__':
    logger.info('🧠 Testing Unified Memory System...')
    results = search_memory('docker')
    logger.info(f"📋 Found {len(results)} memories about 'docker'")
    context = get_context()
    logger.info(f'📊 Session context keys: {list(context.keys())}')
    continue_data = continue_session()
    logger.info(f'🔄 Continuity data: {len(continue_data)} items')
