...
Usage:
    python validate_stability.py [--config CONFIG_PATH] [--stabilize-time SECONDS]
"""
...
# Default paths
DEFAULT_CONFIG_PATH = "main_pc_code/config/startup_config.yaml"
DEFAULT_CLEANUP_SCRIPT = "scripts/cleanup_agents.py"
DEFAULT_SUPERVISOR_SCRIPT = "utils/agent_supervisor.py"
DEFAULT_HEALTH_CHECK_SCRIPT = "main_pc_code/scripts/check_all_agents_health.py"
...
def cleanup_agents():
    ...
    # Try multiple possible locations for the cleanup script
    possible_cleanup_scripts = [
        "scripts/cleanup_agents.py",
        "main_pc_code/scripts/cleanup_agents.py",
        "main_pc_code/NEWMUSTFOLLOW/cleanup_agents.py"
    ]
    
    cleanup_script = None
    for script_path in possible_cleanup_scripts:
        script = Path(script_path).resolve()
        if script.exists():
            cleanup_script = script
            logger.info(f"Found cleanup script at {cleanup_script}")
            break
    
    if cleanup_script is None:
        logger.error("Cleanup script not found in any of the expected locations")
...
def start_agent_supervisor(config_path):
    ...
    # Try multiple possible locations for the supervisor script
    possible_supervisor_scripts = [
        "utils/agent_supervisor.py",
        "main_pc_code/utils/agent_supervisor.py"
    ]
    
    supervisor_script = None
    for script_path in possible_supervisor_scripts:
        script = Path(script_path).resolve()
        if script.exists():
            supervisor_script = script
            logger.info(f"Found supervisor script at {supervisor_script}")
            break
    
    if supervisor_script is None:
        logger.error("Agent Supervisor script not found in any of the expected locations")
        return None
...
def check_agent_health():
    ...
    # Try multiple possible locations for the health check script
    possible_health_check_scripts = [
        "main_pc_code/scripts/check_all_agents_health.py",
        "main_pc_code/NEWMUSTFOLLOW/check_mvs_health.py",
        "scripts/check_all_agents_health.py"
    ]
    
    health_check_script = None
    for script_path in possible_health_check_scripts:
        script = Path(script_path).resolve()
        if script.exists():
            health_check_script = script
            logger.info(f"Found health check script at {health_check_script}")
            break
    
    if health_check_script is None:
        logger.error("Health check script not found in any of the expected locations")
        return None
    
    result = run_command([sys.executable, str(health_check_script)], timeout=120)
