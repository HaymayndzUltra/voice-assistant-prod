...
from src.agents.mainpc.model_manager_agent_gguf_connector import get_instance as get_gguf_connector
...
class ModelManagerAgent(BaseAgent):
    ...
    def __init__(self, port: int = None, **kwargs):
        ...
        # Initialize ZMQ with test ports if provided
    ...
    def _init_zmq(self, test_ports: Optional[Tuple[int, int]] = None):
        ...
        try:
            ...
            # Use test ports if provided, otherwise use configured ports
            self.model_port = test_ports[0] if test_ports else 5555
            self.status_port = test_ports[1] if test_ports else 5556
    ...
    def run(self):
        ...
        try:
            ...
            self.threads.extend([memory_thread, health_thread])
    ...
    def _publish_status_update(self):
        ...
        try:
            ...
            logger.info(f"VRAM usage: {self.current_estimated_vram_used:.2f}MB / {self.vram_budget_mb:.2f}MB")
        except Exception as e:
...
from main_pc_code.utils.config_loader import Config
    ...
    def load_model(self, model_id):
        ...
        if serving_method == 'zmq_service':
            ...
            try:
from src.agents.mainpc.gguf_model_manager import get_instance as get_gguf_manager
    ...
    def _handle_model_requests_loop(self):
        ...
        while self.running:
            try:
                ...
                self.socket.bind(f"tcp://*:{self.model_port}")
                self.logger.info("ZMQ socket reset and rebound")
    ...
    def _init_model_registry(self):
        ...
        try:
            ...
            if pc2_config.get('enabled', False):
                ...
                for service_name, service_config in pc2_config.items():
                    if isinstance(service_config, dict) and service_config.get('enabled', False):
                        ...
                        if port:
                            ...
                            self.models[service_id] = {
                                'type': 'zmq_service',
                                'zmq_address': f"tcp://{pc2_ip}:{port}",
                                'priority': priority,
                                'enabled': True,
                                'estimated_vram_mb': 512  # Default VRAM estimate
                            }
                            self.logger.info(f"Registered PC2 service: {service_id} on port {port}")
            else:
    ...
    def verify_pc2_services(self) -> Dict[str, bool]:
        ...
        try:
            ...
            for service_name, service_config in pc2_config.items():
                ...
                try:
                    # Connect to service
                    health_socket.connect(f"tcp://{pc2_ip}:{port}")
                    ...
                    try:
                        health_socket.disconnect(f"tcp://{pc2_ip}:{port}")
                    except:
                        pass
...
if __name__ == "__main__":
    print("--- MMA: Attempting to initialize and run ModelManagerAgent ---")
    # logger.critical("--- MMA: Attempting to initialize and run ModelManagerAgent ---")  # Commented out for now
    try:
        mma = ModelManagerAgent()
        print("--- MMA: ModelManagerAgent instantiated successfully ---")
        # logger.critical("--- MMA: ModelManagerAgent instantiated successfully ---")  # Commented out for now
...
from common.utils.env_standardizer import get_mainpc_ip, get_pc2_ip, get_current_machine, get_env
    ...
    # (Siguraduhing ang mma.run() ay HINDI MUNA TATAWAGIN para makita natin ang print output bago mag-crash)
    # Ilagay dito ang mma.run() kapag ready na tayong i-test ulit ang full run.
    # For now, we just want to see the diagnostic prints.
    print("--- MMA: End of __main__ block (mma.run() is currently commented out for diagnostics) ---")

    # The following block is commented out for this diagnostic run:
    # try:
    ...
    #     print("--- MMA: ModelManagerAgent initialized. ABOUT TO CALL agent.run() ---")
    #     agent.run()
    ...
    raise SystemExit("MMA_FORCED_EXIT_VERSION_DEBUG_004: This is the CORRECT and LATEST MMA script being executed!")
