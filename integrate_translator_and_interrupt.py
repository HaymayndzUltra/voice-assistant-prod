...
class IntegrationTester:
    ...
    def __init__(self):
        ...
        self.test_results = {'translator_integration': {'status': 'not_tested', 'details': []}, 'interrupt_handler_integration': {'status': 'not_tested', 'details': []}}
        self.processes = []
    ...
    def start_required_services(self):
        ...
        if not self._is_service_running(self.translator_port):
            ...
            if translator_path.exists():
                process = subprocess.Popen([sys.executable, str(translator_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if not self._is_service_running(self.interrupt_handler_port):
            ...
            if interrupt_path.exists():
                process = subprocess.Popen([sys.executable, str(interrupt_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_translator_integration(self):
        ...
        try:
            self.translator_socket.send_string(json.dumps({'action': 'translate', 'text': 'Buksan ang file', 'source_language': 'tl', 'target_language': 'en'}))
            ...
            if response_data.get('status') == 'success' and 'open the file' in response_data.get('translated_text', '').lower():
                ...
                self.test_results['translator_integration']['details'].append({'test': 'direct_connection', 'status': 'success', 'response': response_data})
            else:
                logger.warning(f'Direct translator test failed: {response_data}')
                self.test_results['translator_integration']['details'].append({'test': 'direct_connection', 'status': 'failed', 'response': response_data})
        except Exception as e:
            logger.error(f'Error testing direct translator connection: {e}')
            self.test_results['translator_integration']['details'].append({'test': 'direct_connection', 'status': 'error', 'error': str(e)})
        try:
            self.task_router_socket.send_string(json.dumps({'action': 'process', 'task_type': 'translation', 'text': 'Buksan ang file', 'source_language': 'tl', 'target_language': 'en'}))
            response = self.task_router_socket.recv_string()
            response_data = json.loads(response)
            if response_data.get('status') == 'success':
                logger.info('Task router integration test successful')
                self.test_results['translator_integration']['details'].append({'test': 'task_router_integration', 'status': 'success', 'response': response_data})
            else:
                logger.warning(f'Task router integration test failed: {response_data}')
                self.test_results['translator_integration']['details'].append({'test': 'task_router_integration', 'status': 'failed', 'response': response_data})
        except Exception as e:
            logger.error(f'Error testing task router integration: {e}')
            self.test_results['translator_integration']['details'].append({'test': 'task_router_integration', 'status': 'error', 'error': str(e)})
    ...
    def test_interrupt_handler_integration(self):
        ...
        try:
            self.interrupt_pub_socket.send_string(json.dumps({'type': 'partial_transcription', 'text': 'Stop talking please', 'detected_language': 'en'}))
            time.sleep(1)
            self.tts_socket.send_string(json.dumps({'action': 'status'}))
            ...
            if response_data.get('status') == 'idle' or response_data.get('was_interrupted', False):
                ...
                self.test_results['interrupt_handler_integration']['details'].append({'test': 'interrupt_detection', 'status': 'success', 'response': response_data})
            else:
                logger.warning(f'Interrupt detection test failed: {response_data}')
                self.test_results['interrupt_handler_integration']['details'].append({'test': 'interrupt_detection', 'status': 'failed', 'response': response_data})
        except Exception as e:
            logger.error(f'Error testing interrupt detection: {e}')
            self.test_results['interrupt_handler_integration']['details'].append({'test': 'interrupt_detection', 'status': 'error', 'error': str(e)})
        try:
            self.interrupt_pub_socket.send_string(json.dumps({'type': 'interrupt', 'timestamp': time.time()}))
            time.sleep(1)
            try:
                ...
                if response_data.get('type') == 'interrupt':
                    ...
                    self.test_results['interrupt_handler_integration']['details'].append({'test': 'direct_interrupt', 'status': 'success', 'response': response_data})
                else:
                    logger.warning(f'Direct interrupt test failed: {response_data}')
                    self.test_results['interrupt_handler_integration']['details'].append({'test': 'direct_interrupt', 'status': 'failed', 'response': response_data})
            except zmq.Again:
                logger.warning('No interrupt response received')
                self.test_results['interrupt_handler_integration']['details'].append({'test': 'direct_interrupt', 'status': 'failed', 'error': 'No response received'})
        except Exception as e:
            logger.error(f'Error testing direct interrupt: {e}')
            self.test_results['interrupt_handler_integration']['details'].append({'test': 'direct_interrupt', 'status': 'error', 'error': str(e)})
    ...
    def update_task_router_config(self):
        ...
        try:
            ...
            if not task_router_config_path.exists():
                task_router_config = {'services': {}}
            else:
                with open(task_router_config_path, 'r') as f:
                    task_router_config = json.load(f)
            task_router_config['services']['translator'] = {'name': 'ConsolidatedTranslator', 'port': self.translator_port, 'host': 'localhost', 'capabilities': ['translation', 'language_detection'], 'priority': 1}
    ...
    def update_audio_pipeline_config(self):
        ...
        try:
            ...
            if not audio_config_path.exists():
                audio_config = {'components': {}}
            else:
                with open(audio_config_path, 'r') as f:
                    audio_config = json.load(f)
            audio_config['components']['interrupt_handler'] = {'name': 'StreamingInterruptHandler', 'port': self.interrupt_handler_port, 'host': 'localhost', 'capabilities': ['interrupt', 'streaming'], 'priority': 1}
    ...
    def generate_report(self):
        ...
        report = {'timestamp': time.time(), 'date': time.strftime('%Y-%m-%d %H:%M:%S'), 'translator_integration': self.test_results['translator_integration'], 'interrupt_handler_integration': self.test_results['interrupt_handler_integration'], 'configurations_updated': {'task_router': self.update_task_router_config(), 'audio_pipeline': self.update_audio_pipeline_config()}}
        ...
        logger.info(f"Translator Integration: {self.test_results['translator_integration']['status']}")
        logger.info(f"Interrupt Handler Integration: {self.test_results['interrupt_handler_integration']['status']}")
        logger.info(f"Task Router Config Updated: {report['configurations_updated']['task_router']}")
        logger.info(f"Audio Pipeline Config Updated: {report['configurations_updated']['audio_pipeline']}")
...
def main():
    """Main function."""
    logger.info('=== Translator and Interrupt Handler Integration ===')
