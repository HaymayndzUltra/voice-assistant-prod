...
class MainPCHealthChecker:
    ...
    def __init__(self):
        ...
        self.agents = {}
        self.startup_order = []
        # Define specific launch order
        self.launch_order = ['SystemDigitalTwin', 'MemoryOrchestrator', 'CoordinatorAgent', 'ModelManagerAgent']
        self.processes = {}
        self.running = True
    ...
    def load_agents_from_config(self) -> None:
        ...
        try:
            ...
            # Process only core infrastructure agents
            categories = [
                'monitoring_security',  # Contains SystemDigitalTwin
                'memory_system',        # Contains MemoryOrchestrator
                'core_services'         # Contains ModelManagerAgent and CoordinatorAgent
            ]
            
            # Filter to include only the core infrastructure agents
            filter_agents = ['SystemDigitalTwin', 'MemoryOrchestrator', 'CoordinatorAgent', 'ModelManagerAgent']
            
            # Skip dependency checks for these agents
            skip_dependency_checks = ['TaskRouter']
            
            # Skip other categories for focused testing
            # Full list: 'core_services', 'main_pc_gpu_services', 'emotion_system',
                ...
                if category in config:
                    for agent_config in config[category]:
                        ...
                        if name and script_path and port:
                            ...
                            if 'filter_agents' in locals() and name not in filter_agents:
                                continue
    ...
    def analyze_agent_health_check_mechanisms(self) -> None:
        ...
        for name, agent in self.agents.items():
            ...
            if name == "CoordinatorAgent":
                ...
                agent.zmq_health_check_method["success_value"] = "healthy"  # Expect detailed health status
                logger.info(f"Set custom health check for CoordinatorAgent to expect detailed 'healthy' status.")
    ...
    def _check_zmq_health(self, agent_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to {agent_name} health check port {port} at {connect_addr}")
            
            # Send health check request
            socket.send_json(request_data)
            logger.info(f"Sent health check request to {agent_name}: {request_data}")
            ...
            if poller.poll(HEALTH_CHECK_TIMEOUT):
                try:
                    ...
                    logger.info(f"Received health check response from {agent_name}: {response}")
                    
                    # Check if response indicates health
                    if success_key in response and response[success_key] == success_value:
                    ...
                    logger.error(f"Error processing response from {agent_name}: {e}")
                    agent.status = "UNHEALTHY"
                    return False
            else:
            ...
            logger.error(f"Error connecting to {agent_name} health check port {port}: {e}")
            agent.status = "UNREACHABLE"
            return False
        finally:
            socket.close()
    ...
    def run(self) -> int:
        ...
        try:
            ...
            # Use our custom launch order instead of determining it automatically
            self.startup_order = self.launch_order
