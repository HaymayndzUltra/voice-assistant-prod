...
MODEL_MANAGER_HOST = config.get('zmq.model_manager_host', '192.168.1.27')  # Main PC's IP address
...
class RemoteConnectorAgent:
    def __init__(self):
        ...
        self.model_manager_port = config.get('zmq.model_manager_port', 5556)
        ...
        try:
            ...
            return {
                ...
                "cached": False
                    ...
                    logger.warning(f"Request failed with status code {response.status_code} (attempt {attempt + 1}/{max_retries})")
                    if attempt < max_retries - 1:
                        ...
                        return {
                            "status": "error",
                            "model": model,
                            "message": f"Request failed after {max_retries} attempts. Last status code: {response.status_code}"
                        }
