...
class TranslationPipeline:
    ...
    def _handle_connection_error(self, engine: str, error: Exception) -> Dict[str, Any]:
        ...
        try:
            if engine == 'nllb':
                self.nllb_socket.connect(f"tcp://localhost:{self.config['engines']['nllb']['port']}")
            elif engine == 'phi':
...
class TranslatorServer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Try to bind to the configured port, fall back to alternatives if needed
        try:
            self.socket.bind(f"tcp://{ZMQ_BIND_ADDRESS}:{ZMQ_PORT}")
            logger.info(f"Bound to port {ZMQ_PORT}")
            ...
            if "Address in use" in str(e):
                ...
                for alt_port in alt_ports:
                    try:
                        self.socket.bind(f"tcp://{ZMQ_BIND_ADDRESS}:{alt_port}")
                    ...
                    raise RuntimeError("Cannot bind to any port")
            else:
                logger.error(f"Error binding socket: {e}")
                raise RuntimeError(f"Cannot bind to port {ZMQ_PORT}")
        self.start_time = time.time()
    ...
    def _handle_health_check(self) -> Dict[str, Any]:
        ...
        health.update({
            'api_version': self.api_version,
            'uptime': time.time() - self.start_time,
            'memory_stats': self.pipeline.memory_stats,
