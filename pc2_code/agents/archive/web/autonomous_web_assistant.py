...
class AutonomousWebAssistant:
    ...
    def __init__(self, zmq_port=AUTONOMOUS_WEB_ASSISTANT_PORT):
        ...
        logger.info(f"[AutonomousWebAssistant] Agent started on port {zmq_port} (REP socket)")
        
        # Connect to Enhanced Model Router (port 5601)
        ...
        try:
            ...
            self.model_router.connect(f"tcp://localhost:{ENHANCED_MODEL_ROUTER_PORT}")
        ...
        try:
            ...
            logger.info(f"Connected to Enhanced Web Scraper on port {ENHANCED_WEB_SCRAPER_PORT}")
