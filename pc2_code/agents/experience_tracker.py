...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/experience_tracker.log'),
...
if __name__ == "__main__":
    ...
    agent = ExperienceTrackerAgent(
        port=getattr(args, 'port', 7112),
        health_port=getattr(args, 'health_port', 7113),
        episodic_agent_port=getattr(args, 'episodic_agent_port', 7106)
    )
    try:
        agent.run()
