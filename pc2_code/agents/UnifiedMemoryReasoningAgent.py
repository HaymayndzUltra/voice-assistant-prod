...
# Add project root to Python path for easier imports of sibling/parent modules
current_dir = Path(__file__).resolve().parent
project_root = current_dir.parent.parent
if str(project_root) not in sys.path:
    sys.sys.path.insert(0, str(project_root))

# Import base agent and configuration systems
from main_pc_code.src.core.base_agent import BaseAgent
from pc2_code.agents.utils.config_loader import Config # Assuming this Config class is intended for PC2
from main_pc_code.utils.config_loader import load_config as load_main_config # Renamed to avoid conflict
...
BIND_ADDRESS = network_config.get("bind_address", '0.0.0.0') # Default provided in get_env, but explicitly here for clarity
...
class UnifiedMemoryReasoningAgent(BaseAgent):
    ...
    def __init__(self, port: int = None):
        ...
        # The 'port' argument to __init__ is effectively ignored if env/config is used
        ...
        if self.secure_zmq and SECURE_ZMQ_AVAILABLE:
            ...
            except Exception as e: # This remains 'Exception' as it's a runtime error in ZMQ config
        ...
        try:
            ...
            # Continue even if health check socket fails, but log it
        ...
        # self.start_time is already set by BaseAgent init
        ...
        # Dictionary to store main PC service connections
        self.main_pc_connections: Dict[str, zmq.Socket] = {}
    ...
    def _register_with_service_discovery(self):
        ...
        try:
            ...
            try:
                ...
                s.connect(("8.8.8.8", 80))  # Doesn't have to be reachable for IP
                my_ip = s.getsockname()[0]
                s.close()
            except Exception as e:
                logger.warning(f"Failed to determine IP address for registration: {e}, using configured host ({self.host})")
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    logger.info(f"Registering with service discovery as {self.name} at {my_ip}:{self.port} (attempt {attempt} of {max_retries})")
            ...
            logger.error(f"Fatal error registering with service discovery: {e}")
    ...
    def _interrupt_monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.interrupted:
                    time.sleep(0.5) # Give some time for main loop to react
                    self.interrupted = False # Reset the flag
    ...
    def _handle_interrupt(self):
        ...
        try:
            ...
            # Implement logic to stop any long-running, interruptible operations here.
            # For a memory agent, this might mean stopping a large sync or query.
            # For this simple in-memory store, the impact is minimal.
    ...
    def _apply_decay(self):
        ...
        for memory_id in memory_ids:
            try:
                ...
                # If strength is below threshold, mark as forgotten but keep for recovery
                if new_strength < MIN_MEMORY_STRENGTH:
                    memory["forgotten"] = True
                    forget_count += 1
                    logger.debug(f"Memory {memory_id} forgotten due to low strength: {new_strength:.2f}")
        ...
        try:
            ...
            if memory_id:
                if memory_id in self.memory_store:
                    ...
                    # Update last_accessed timestamp
                    memory["last_accessed"] = time.time()
                    self.memory_store[memory_id] = memory # Ensure updated in store
            ...
            # If list of memory_ids is provided
                ...
                for mid in memory_ids:
                    if mid in self.memory_store:
                        ...
                        # Update last_accessed timestamp
                        memory["last_accessed"] = time.time()
                        self.memory_store[mid] = memory # Ensure updated in store
    ...
    def _handle_sync_memories(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not memories:
                return {
                    "status": "SUCCESS", # Changed from ERROR, as an empty sync might be valid
                    "message": "No memories provided for synchronization, nothing to do",
                    "data": {
                        "synced_count": 0,
                        "total_memories": len(self.memory_store)
                    }
            ...
            for memory in memories:
                ...
                if not memory_id:
                    logger.warning("Received memory without ID during sync, skipping.")
                    continue
                ...
                if memory_id in self.memory_store:
                    ...
                    # Skip if local version is newer or same
    ...
    def _handle_health_check(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            message: The command message (optional)
        ...
        # The main health check method is _get_health_status inherited from BaseAgent
        # but the original code had a separate _health_check. Let's merge them.
        return self._get_health_status()
    
    def _get_health_status(self) -> Dict[str, Any]:
        """
        Perform health check and return status. Overrides BaseAgent's method.
        """
        base_status = super()._get_health_status() # Get basic status from BaseAgent
        ...
        base_status.update({
            ...
            "health": {
                "overall": True,  # Assume true unless specific checks fail
                "decay_thread_alive": decay_thread_alive,
                "interrupt_thread_alive": interrupt_thread_alive,
                "memory_store_size": len(self.memory_store),
                "is_running": self.running # Add agent's running status
            },
            "metrics": {
                ...
                "active_memories": sum(1 for m in self.memory_store.values() if not m.get("forgotten", False)),
                "forgotten_memories": sum(1 for m in self.memory_store.values() if m.get("forgotten", False)),
                "total_reinforcements": self.memory_stats["total_reinforcements"],
                "total_decays": self.memory_stats["total_decays"],
    ...
    def _health_check_loop(self):
        ...
        while self.running:
            try:
                ...
                if poller.poll(100):  # 100ms timeout
                    # Receive request (don't care about content, just that a request arrived)
                    _ = self.health_socket.recv()
                    
                    # Get health data using the unified _get_health_status method
    ...
    def run(self):
        """Run the agent's main loop and start background threads."""
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if poller.poll(100):  # 100ms timeout
                        ...
                        try:
                            ...
                            if not self.interrupted:
                                ...
                                if response.get("status", "").lower() == "success": # Check for both "SUCCESS" and "success"
                                ...
                                response = {
                                    ...
                                    "command": message.get("command", "N/A")
                                }
                                # Interruption counts as a failed request for the original operation
                                self.failed_requests += 1
                    ...
                    logger.error(f"Error in main processing loop: {e}", exc_info=True) # Log traceback
                    time.sleep(1)
            ...
            self.cleanup() # Call the unified cleanup method
            
    def cleanup(self):
        """Clean up resources before shutdown. Overrides BaseAgent's method."""
        logger.info(f"Cleaning up resources for {self.name}...")
        
        # Set running flag to false to signal all threads to stop
        self.running = False
        
        # Wait for threads to finish (with a timeout)
        threads = [self.decay_thread, self.interrupt_thread, self.health_thread]
        ...
        # Close main PC connections
        for service_name, sock in self.main_pc_connections.items():
            try:
                if not sock.closed:
                    sock.close()
    ...
    def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process message and return response. This method serves as the main command router.
        ...
        Returns:
            Response message
        """
        logger.info(f"Processing message: {message.get('command', 'UNKNOWN COMMAND')}")
        ...
        if handler:
            try:
                ...
                return {
                    "status": "ERROR",
                    "message": f"Internal server error processing command '{command}': {str(e)}"
                }
        else:
    ...
    def connect_to_main_pc_service(self, service_name: str) -> Optional[zmq.Socket]:
        ...
        if service_name in self.main_pc_connections:
            # You might want to add a check here if the existing socket is still valid/connected
            # For simplicity, returning existing socket.
            return self.main_pc_connections[service_name]
            
        # Create a new socket for this connection
        socket = self.context.socket(zmq.REQ)
        
        # Apply secure ZMQ if available and enabled
        if self.secure_zmq and SECURE_ZMQ_AVAILABLE:
            try:
                ...
                # Fallback to non-secure or raise? For now, we continue with non-secure.
                # In a real system, you might want to stop if security is mandatory.
        ...
        try:
            ...
            logger.error(f"Failed to connect to {service_name} at {connect_address}: {e}")
