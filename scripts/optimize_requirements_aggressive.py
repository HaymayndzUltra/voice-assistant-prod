#!/usr/bin/env python3
"""
Aggressive requirements.txt optimization script for Docker containers
Removes ANY unused dependencies, even single packages
"""
import pathlib
import ast
import re
...
def optimize_requirements_file(agent_dir: pathlib.Path) -> Dict:
    ...
    if not current_requirements:
        return {"status": "empty_requirements"}
    ...
    # AGGRESSIVE: Write optimized requirements if there are ANY unused packages
    if len(unused) > 0:  # Changed from > 2 to > 0
        ...
        with req_file.open('w') as f:
            f.write("# Auto-optimized requirements.txt (aggressive mode)\n")
...
def main():
    """Main function to aggressively optimize all requirements.txt files"""
    print("Starting AGGRESSIVE requirements.txt optimization...")
        ...
        if result["status"] == "analyzed":
            ...
            if result["action"] == "optimized":
                ...
                if result["unused_packages"]:
                    print(f"    Removed: {', '.join(result['unused_packages'])}")
            else:
    ...
    results_file = ROOT / "scripts" / "requirements_optimization_aggressive_results.json"
