...
# WP-10 Authentication Integration for streaming_speech_recognition
...
class StreamingSpeechRecognitionAuthenticationIntegration:
    """Authentication integration for streaming_speech_recognition"""
    ...
    def __init__(self):
        ...
        key_id, api_key = self.security_manager.create_api_key(
            name="streaming_speech_recognition_service_key",
            permissions={
        ...
        print(f"Generated API key for streaming_speech_recognition: {key_id}")
    ...
    async def authenticate_request(self, request_headers: dict) -> SecurityContext:
        ...
        if signature and timestamp:
            context = await self.security_manager.authenticate(
                ...
                {
                    "method": "POST",
                    "path": "/api/streaming_speech_recognition",
                    "body": "",  # Request body
                    "timestamp": timestamp,
                    "signature": signature
                }
    ...
    async def verify_request_integrity(self, request_data: dict, 
        ...
        return self.security_manager.hmac_validator.verify_signature(
            method="POST",
            path=f"/api/streaming_speech_recognition",
            body=orjson.dumps(request_data, sort_keys=True).decode(),
            timestamp=timestamp,
            received_signature=signature
        )

# Example usage:
# auth_integration = StreamingSpeechRecognitionAuthenticationIntegration()
# 
